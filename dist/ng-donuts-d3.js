'use strict';
angular.module('ngDonutsD3', []).directive('myDirective', function () {
  return {
    restrict: 'EAC',
    scope: true,
  ...
  function ($window) {
    return {
      ...
      link: function postLink(scope, element, attrs) {
        ...
        svg.style('width', '100%').style('height', '100%').attr('viewBox', '0, 0, ' + largo + ', ' + largo).attr('preserveAspectRatio', 'xMinYMin');
        var arco, resu, textoCentro;
        var amplitud = largo * 0.05;
        var dAmplitud = Math.ceil(amplitud * 0.3);
        var radio = largo / 2 - dAmplitud;
        var arc = d3.svg.arc().innerRadius(radio - amplitud).outerRadius(radio).startAngle(0);
        ...
        var build = function build() {
          svg.select('g').remove();
          var g = svg.append('g').attr('transform', 'translate(' + largo / 2 + ',' + largo / 2 + ')');
          var arcoFondo = d3.svg.arc().startAngle(0).endAngle(τ).innerRadius(radio - amplitud + dAmplitud).outerRadius(radio - dAmplitud);
          arco = d3.svg.arc().startAngle(0).endAngle(0).innerRadius(radio - amplitud).outerRadius(radio);
          // .cornerRadius(20);//modified d3 api only
          //Añadir primero el arco de fondo
          g.append('path').attr('d', arcoFondo).attr('fill', function () {
            return '#eee';
          });
          //Añadir arco con porcentaje
          resu = g.append('path').datum({ endAngle: 0 }).attr('d', arco).attr('fill', function () {
          ...
          /*Arco animacion*/
          resu.transition().duration(2000).call(arcTween, scope.porcentaje / 100 * τ);  /*Arco animacion*/
        };
        build();
